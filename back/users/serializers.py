    ...
    def create(self, validated_data):
        ...
        try:
            ...
            if est_from_req is not None:
                update_row['estuserid'] = est_from_req
            else:
                default_est = environ.get('SUPABASE_DEFAULT_ESTUSERID')
                if default_est is not None and default_est != '':
                    try:
                        update_row['estuserid'] = int(default_est)
                    except Exception:
                        update_row['estuserid'] = default_est
            resp = admin.schema(schema).table(table_name).update(update_row).eq('userid', str(user.id)).execute()
            logger.warning('Supabase update into %s.%s result: %s', schema, table_name, getattr(resp, 'data', None))
        except Exception as e:
