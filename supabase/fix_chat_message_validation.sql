-- Script para arreglar la validación de chat_messages
-- Ejecutar en Supabase SQL Editor

-- 1. Verificar si existe la función validate_chat_message_file
SELECT 
    routine_name,
    routine_type,
    routine_definition
...
-- 2. Verificar si hay triggers que usan esta función
SELECT 
    trigger_name,
    event_manipulation,
    action_statement,
...
-- 3. Deshabilitar temporalmente el trigger
-- (Ejecutar solo si el paso 2 muestra un trigger)
-- ALTER TABLE public.chat_messages DISABLE TRIGGER [nombre_del_trigger];

-- 4. Eliminar la función si es necesario
-- (Ejecutar solo si el paso 1 muestra la función)
-- DROP FUNCTION IF EXISTS public.validate_chat_message_file();

-- 5. Verificar que no hay más restricciones
SELECT 
    tc.constraint_name,
    tc.constraint_type,
    cc.check_clause
